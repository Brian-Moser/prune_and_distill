...
def main(args):
    ...
    # After the build_dataset function call where indices_class is defined
    total_training_images = sum(len(indices_class[c]) for c in range(num_classes))
    print(f"Total number of training images across all classes: {total_training_images}")
    ...
    latents, f_latents, label_syn = prepare_LDM_latents_encode(channel=channel, num_classes=num_classes, im_size=im_size,
    ...
    for it in range(args.Iteration+1):
        ...
        if it == args.Iteration: # only record the final results
            ...
            torch.save({'data': data_save, 'accs_all_exps': accs_all_exps, }, os.path.join(args.save_path, 'res_dm_%s_%s_%s_%dipc.pt'%(args.dataset, args.subset, args.model, args.ipc)))
...
if __name__ == '__main__':
    if __name__ == '__main__':
        ...
        parser.add_argument('--lr_w', type=float, default=1, help='learning rate for updating synthetic latent w') # .01
