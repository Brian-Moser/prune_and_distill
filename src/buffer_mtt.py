...
from utils import get_loops, get_dataset, get_network, get_eval_pool, get_daparam, TensorDataset, epoch, ParamDiffAug
import copy
...
def main(args):
    ...
    args.batch_test = 128
    (channel, im_size, num_classes, class_names,
      mean, std, dst_train, dst_test, testloader,
    ...
    if args.dataset != "ImageNet" or True:
        ...
        json_files = {}
        for c in range(1000):
            json_file = os.path.join(args.json_path, f'class_{c}_top_{args.percent}_{args.order}.json')
            try:
        ...
        for i in tqdm(range(len(dst_train))):
            ...
            if args.dataset == "CIFAR10":
                img_path, class_label = sample[0], sample[1]
            else:
                img_path, class_label = dst_train.dataset.samples[dst_train.indices[i]]  # Use samples to access image paths
            class_idx = class_map[class_label]

            if args.percent == 100 or img_path in json_files[class_label]:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--json_path', type=str, default='/netscratch/bmoser/pruning_and_distillation/imagenet_losses', help='Path to the JSON files containing top X% image lists')
    parser.add_argument('--percent', type=int, default=10, help='Top X percent of images to use')
    parser.add_argument('--order', type=str, choices=['asc', 'desc'], default='asc', help='Order to load images by loss (asc/desc)')
