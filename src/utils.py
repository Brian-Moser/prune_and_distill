...
def get_dataset(dataset, data_path, batch_size=1, res=None, args=None):
    ...
    if dataset == 'CIFAR10':
        ...
        transform = transforms.Compose([transforms.Resize((128,128)), transforms.ToTensor(), transforms.Normalize(mean=mean, std=std)])
        ...
        ##
        dst_train = datasets.ImageFolder(data_path + '/train', transform=transform)
        dst_train_dict = {
            c: torch.utils.data.Subset(dst_train, np.squeeze(np.argwhere(np.equal(dst_train.targets, config.img_net_classes[c]))))
        ...
        dst_test = datasets.ImageFolder(data_path + '/val_folders', transform=transform)
...
# ! the original repo GLaD did not have this function.
# ! It is obtained from the original MTT repo https://github.com/GeorgeCazenavette/mtt-distillation/blob/main/utils.py#L455
