...
def main(args):
    ...
    channel, im_size, num_classes, _, _, _, dst_train, _, testloader, _, class_map, class_map_inv = get_dataset(args.dataset, args.data_path, args.batch_real, args.res, args=args)
    ...
    images_all, _, indices_class = build_dataset(dst_train, class_map, num_classes, args.json_path, args.percent, args.order)
    ...
    for it in range(0, args.Iteration+1):
        ...
        if it in eval_it_pool:
            save_this_it = eval_loop_ldm_encode(latents=latents, f_latents=f_latents, label_syn=label_syn, G=G, sampler=sampler, best_acc=best_acc, best_std=best_std, testloader=testloader,
        ...
        if args.space == "wp":
            with torch.no_grad():
                syn_images = torch.cat([ldm_latent_to_im_encode(sampler, G, syn_image_split.detach(), f_latents_split.detach(), args=args).detach() 
                                        for syn_image_split, f_latents_split in zip(torch.split(syn_images, args.sg_batch),
        ...
        for il in range(args.syn_steps):
            ...
            gradients = 2  * torch.autograd.grad( (single_term + square_term) @ grad / param_dist, original_x_list[il])
            ...
            with torch.no_grad():
                syn_images_grad[indices_chunks_copy[il]] += gradients[0]

        # ---------end of computing input image gradients and learning rates--------------
        #del per_batch_loss
        del w, output, ce_loss, grad, square_term, single_term,  gradients, student_net, w_expanded, forward_params, forward_params_expanded
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_start_epoch', type=int, default=2)
    parser.add_argument('--max_files', type=int, default=None)
    parser.add_argument('--max_experts', type=int, default=None)
    parser.add_argument('--expert_epochs', type=int, default=2, help='how many expert epochs the target params are') # 3
    parser.add_argument('--syn_steps', type=int, default=10, help='how many steps to take on synthetic data') # 20
    parser.add_argument('--norm_train', type=str, default="instancenorm") # was not in original implementation but is actually needed?

    parser.add_argument('--lr_img', type=float, default=10000, help='learning rate for pixels or f_latents')
    parser.add_argument('--lr_w', type=float, default=1000, help='learning rate for updating synthetic latent w') # 10
    ...
    parser.add_argument('--unconditional_guidance_scale', type=float, default=3.0, help='learning rate for gan weights')
